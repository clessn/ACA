...
# Dependent variables
dvs <- c(
  "tradeoff_invest_green_num", # "Control, increase green \n economy spending"
  "tradeoff_taxes_green_num", #"Higher taxes"
  "tradeoff_cutting_for_green_num", # "Cuts"
  "tradeoff_debt_green_num" # "More debt
)
...
ivs <- c(
  ...
  "budget_spend_prio_climateChange_bin" # Prioritization of climate change spending
)
...
plot2 <- ggplot(plot_data, aes(x = treatment, y = estimate, color = model, shape = model)) +
  ...
  scale_y_continuous()  + #scale_y_continuous(limits = c(0, 1), breaks = seq(0, 1, 0.2)) +
  labs(
    x = NULL,
    y = "Predicted Support \n(0–1 scale)",
    color = "Model Type",
...
ggsave(
  filename = "graphs/avgSupport_experimentGE_reference.png",
  plot     = plot2,
  width    = 10,
  height   = 8,
  dpi      = 300
