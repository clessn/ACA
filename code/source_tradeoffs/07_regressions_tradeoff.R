...
walk(names(batteries), function(bat_name) {
  ...
  # Primary: Logit AME — filtered to hypothesis variables for figures only
  coef_lp <- coef_tradeoff |> dplyr::filter(battery == bat_name, question == paste0(bat_name, "_pref_logit"),    term %in% hyp_vars)
  coef_li <- coef_tradeoff |> dplyr::filter(battery == bat_name, question == paste0(bat_name, "_intense_logit"), term %in% hyp_vars)
  coef_pp <- coef_tradeoff |> dplyr::filter(battery == bat_name, question == paste0(bat_name, "_pref_lpm"),      term %in% hyp_vars)
  coef_pi <- coef_tradeoff |> dplyr::filter(battery == bat_name, question == paste0(bat_name, "_intense_lpm"),   term %in% hyp_vars)
